            ...
            {dropdownOpen && (
              <div className={`absolute right-0 top-12 w-48 rounded-2xl shadow-2xl overflow-hidden z-50 ${isDark ? 'glass' : 'glass-light'}`}>
                <div className={`px-4 py-3 border-b ${isDark ? 'border-white/10' : 'border-slate-200'}`}>
                  <p className={`text-xs ${isDark ? 'text-white/40' : 'text-slate-400'}`}>Signed in as</p>
                  <p className={`font-semibold text-sm truncate ${isDark ? 'text-white' : 'text-slate-800'}`}>{user?.name}</p>
                  <p className={`text-xs truncate ${isDark ? 'text-white/40' : 'text-slate-400'}`}>{user?.email}</p>
                </div>
                <button
                  onClick={handleLogout}
                  className={`w-full flex items-center gap-2 px-4 py-3 text-sm transition ${isDark ? 'text-red-400 hover:bg-red-500/10' : 'text-red-500 hover:bg-red-50'}`}
                >
                  <LogOut size={15} />
                  Logout
                </button>
