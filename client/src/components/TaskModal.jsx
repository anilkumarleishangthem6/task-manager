...
const TaskModal = ({ isOpen, onClose, onSubmit, editTask }) => {
  ...
  // Shared input styling based on current theme
  const inputClass = `w-full px-4 py-3 rounded-xl text-sm outline-none transition border border-transparent focus:border-purple-500 ${
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <button
            ...
            className={`p-2 rounded-xl transition ${
              isDark ? 'glass text-white/50 hover:text-white' : 'glass-light text-slate-400 hover:text-slate-600'
            }`}
          >
            <X size={18} />
          </button>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button
              type="submit"
              disabled={loading}
              className="flex-1 gradient-btn text-white font-semibold py-3 rounded-xl shadow-lg disabled:opacity-60"
            >
